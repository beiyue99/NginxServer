...
void FunccreateSocket(struct socketinfo &clientproc, int count)
{
	...
	for (k1 = 0; k1 < count; k1++)
	{
		...
		{			
			...
			{
				int reco;
				reco = setsockopt(clientproc.sClient[k1], SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const char*>(&iRecvTimeOut), sizeof(int));
				reco = setsockopt(clientproc.sClient[k1], SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const char*>(&iRecvTimeOut), sizeof(int));
...
unsigned __stdcall ScanThread(LPVOID lppassin_struct)
{
	...
	if (!bFind)
	{
		AfxMessageBox(_T("线程没有找到自己的创建编号"));
		_endthreadex(0);
		return -1;
	}
	//i就是编号
...
void CMFCApplication4Dlg::OnBnClickedBtnPause()
{
	...
	{
		g_ifpausetest = 1;
		m_pausebtn.SetWindowText(_T("继续测试"));

	}
	else
	{
		g_ifpausetest = 0;
		m_pausebtn.SetWindowText(_T("暂停测试"));
	}
}
...
void CMFCApplication4Dlg::OnBnClickedButton5()
{
	...
	if (g_ifstopmoniduan == 1)
	{
		...
		m_btnss.SetWindowText(_T("开启模拟断线"));
	}
	else
	{
		InterlockedIncrement(&g_ifstopmoniduan);
		m_btnss.SetWindowText(_T("暂停模拟断线"));
	}
}
...
{		
	...
	m_list.SetCurSel(m_list.AddString((LPCTSTR)"---------------------------------------------\n")); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	DWORD liushimilsec = GetTickCount() - g_beginmillsecond;  //流逝时间
	m_list.SetCurSel(m_list.AddString(_T("这是client端程序,按包头+灵活包体格式发送数据包,服务器按同样格式接收数据包!\n"))); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	m_list.SetCurSel(m_list.AddString(_T("server端收到正确的包后入队列,触发入队列线程池提取队列消息,立即将收到的消息直接完整的扔到发送队列,触发出队列线程池并发送消息.\n"))); if (m_list.GetCount() > 500) m_list.DeleteString(0);


	sprintf_s(buffer, sizeof(buffer), "系统运行了=%ld秒\n", int(liushimilsec / 1000)); m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	...
	if (chushu > 0)
	{
		...
		m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	}
	sprintf_s(buffer, sizeof(buffer), "当前连接的数量=%ld\n", g_connecting); m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	sprintf_s(buffer, sizeof(buffer), "总共连接成功次数=%ld\n", g_connectsocketsucc); m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	sprintf_s(buffer, sizeof(buffer), "创建socket错误次数=%ld\n", g_createsocketerror); m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	sprintf_s(buffer, sizeof(buffer), "Connect服务器失败次数=%ld,最后一次连接错误号=%ld\n", g_connectsocketerror, g_connectsocketerrorlastErrorinfo); m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	sprintf_s(buffer, sizeof(buffer), "Send失败次数=%ld,最后一次Send错误号=%ld\n", g_sendsocketerror, g_sendsocketerrorlastErrorinfo); m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	sprintf_s(buffer, sizeof(buffer), "Recv失败次数=%ld,最后一次Recv错误号=%ld\n", g_recvsocketerror, g_recvsocketerrorlastErrorinfo); m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	if (chushu > 0)
	{
		sprintf_s(buffer, sizeof(buffer), "Send完成的包数量=%ld,平均每秒Send包数量=%ld\n", g_sendokcount, (int)(g_sendokcount / chushu)); m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
		sprintf_s(buffer, sizeof(buffer), "Recv完成的包数量=%ld,平均每秒Recv包数量=%ld\n", g_recvokcount, (int)(g_recvokcount / chushu)); m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	}
	sprintf_s(buffer, sizeof(buffer), "远程莫名关闭的连接数量=%ld\n", g_remoteclosesocket); m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
	sprintf_s(buffer, sizeof(buffer), "模拟断开的socket数量=%ld\n", g_closesocketcount); m_list.SetCurSel(m_list.AddString((LPCTSTR)buffer)); if (m_list.GetCount() > 500) m_list.DeleteString(0);
