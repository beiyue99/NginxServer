...
void Widget::on_sendButton_clicked()
{
    if (socket->state() != QAbstractSocket::ConnectedState) {
        QMessageBox::critical(this, tr("Error"), tr("Socket is not connected yet!"));
        return;
    }
...
void Widget::on_recvButton_clicked()
{
    ...
    std::vector<char> recvBuffer(100000);  // 使用 std::vector 分配内存
...
{
    ui->connect_Button->setEnabled(false);  // 连接成功后启用按钮
    ui->connect_Button->setText("连接成功!");
...
{
    ui->connect_Button->setEnabled(true);  // 连接失败后启用按钮
...
void Widget::on_floodButton_clicked()
{
    ...
    QMessageBox::information(this, tr("Success"), tr("Will Send 12 data packets!"));
