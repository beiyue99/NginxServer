...
int main(int argc, const char *argv[]) {
  ...
  // 持续检测
  while (1) {
    int num = epoll_wait(epfd, evs, size,
                         -1); //返回就绪文件描述符个数，储存在evs数组里
    for (int i = 0; i < num; ++i) {
      // 取出当前的文件描述符
      ...
      if (curfd == lfd) {
        ...
        if (len == 0) {
          ...
          printf("client say: %s\n", buf);
          /*        如果 buf 的最后一个字符不是空字符(\0)，则在使用 printf
             函数输出时可能会导致乱码或意外的结果。 使用 write 函数直接输出 buf
             的内容，不依赖字符串的结尾空字符。因此，即使最后一个字符不是空字符，
                    也不会出现乱码，write 函数会按照指定的长度输出。
                    为了避免出现乱码或意外结果，确保 buf
             是以空字符结尾的字符串或者明确指定要输出的字节数。*/
